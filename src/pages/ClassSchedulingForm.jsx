...
const ClassSchedulingForm = () => {
  ...
  const courses = ["Bachelor of Technology", "Master of Technology", "Bachelor of Science", "Master of Science", "Bachelor of Business Administration", "Master of Business Administration"];
  const years = ["1st Year", "2nd Year", "3rd Year", "4th Year"];
  const subjects = ["Data Structures", "Algorithms", "Database Management", "Operating Systems", "Computer Networks", "Software Engineering", "Machine Learning", "Artificial Intelligence"];
  const academicBlocks = Array.from({ length: 12 }, (_, i) => `AB-${i + 1}`);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.course} onValueChange={(value) => updateFormData("course", value)}>
                    ...
                    <SelectContent>{courses.map(course => <SelectItem key={course} value={course}>{course}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              ...
              <div className="border-t pt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Select value={formData.academicBlock} onValueChange={(value) => updateFormData("academicBlock", value)}>
                      <SelectTrigger className="h-12"><SelectValue placeholder="Select Academic Block" /></SelectTrigger>
                      <SelectContent>{Array.from({ length: 12 }, (_, i) => `AB-${i+1}`).map(block => <SelectItem key={block} value={block}>{block}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input placeholder="e.g., AB-1-101" value={formData.roomNumber} onChange={e => updateFormData("roomNumber", e.target.value)} className="h-12" />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Topic of Lecture</Label>
                    <Input placeholder="e.g., Introduction to Binary Trees" value={formData.topic} onChange={e => updateFormData("topic", e.target.value)} className="h-12" />
                  </div>
                </div>
              </div>

              <div className="flex justify-center pt-8">
                <Button type="submit" size="lg" className="bg-gradient-primary hover:opacity-90 transition-opacity px-12 py-3 text-lg font-semibold shadow-elegant">
                  Schedule Class
                </Button>
              </div>
            </form>
